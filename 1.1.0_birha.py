...
class _AxiomStoreBase:
    ...
    def _read_all_sheets(self) -> Dict[str, pd.DataFrame]:
        ...
        with pd.ExcelFile(self.store_path) as excel:
    ...
    def _prepare_frame_for_write(self, sheet: str, df: pd.DataFrame) -> pd.DataFrame:
        expected = list(self.SHEET_SCHEMAS[sheet])
        working = df.copy()
        for col in expected:
            if col not in working.columns:
                working[col] = pd.Series([pd.NA] * len(working), dtype="object")
        working = working[expected]
        int_columns = self.INT_COLUMNS.get(sheet, [])
    ...
    def _write_all_sheets(self, sheets: Dict[str, pd.DataFrame]) -> None:
        ...
        for sheet in self.SHEET_SCHEMAS:
            df = sheets.get(sheet)
            if df is None:
                df = pd.DataFrame(columns=self.SHEET_SCHEMAS[sheet])
            prepared[sheet] = self._prepare_frame_for_write(sheet, df)
        with pd.ExcelWriter(self.store_path, engine="openpyxl") as writer:
            for sheet, df in prepared.items():
                df.to_excel(writer, sheet_name=sheet, index=False)

    def _frame_to_records(self, sheet: str, df: pd.DataFrame) -> List[dict]:
        records: List[dict] = []
    ...
    def _export_sheet_to_csv(self, sheet: str, csv_path: Union[str, Path]) -> Path:
        ...
        frame = self._read_all_sheets()[sheet]
        frame = self._prepare_frame_for_write(sheet, frame)
...
class AxiomsStore(_AxiomStoreBase):
    ...
    def find_axioms(self, query: str) -> List[dict]:
        ...
        matched = frame[mask]
        return self._frame_to_records("Axioms", matched)
    ...
    def update_axiom(self, axiom_id: str, **fields: Union[str, None]) -> Optional[dict]:
        ...
        sheets = self._read_all_sheets()
        frame = sheets["Axioms"]
        indices = frame.index[frame["axiom_id"] == axiom_id].tolist()
        if not indices:
            return None
        idx = indices[0]
...
class AxiomDescStore(_AxiomStoreBase):
    ...
    def list_descriptions(
        ...
        filtered = frame[mask]
        return self._frame_to_records("AxiomDescriptions", filtered)
...
class AxiomContribStore(_AxiomStoreBase):
    ...
    def link_contribution(
        ...
        mask = (frame["axiom_id"] == axiom_id) & (frame["verse_key"] == verse_key)
        timestamp = self._timestamp()
        if mask.any():
    ...
    def list_contributions(
        ...
        filtered = frame[mask]
        return self._frame_to_records("AxiomContributions", filtered)
...
class AxiomKeywordsStore(_AxiomStoreBase):
    ...
    def add_keywords(
        ...
        existing = frame[
            (frame["axiom_id"] == axiom_id) & (frame["bucket"] == bucket)
        ]
        existing_keys = {str(val).lower() for val in existing["keyword"].dropna()} if not existing.empty else set()
    ...
    def list_keywords(self, axiom_id: str, bucket: Optional[str] = None) -> List[dict]:
        ...
        filtered = frame[mask]
        return self._frame_to_records("AxiomKeywords", filtered)
...
class AxiomWorkqueueStore(_AxiomStoreBase):
    ...
    def list_queue(self, status: Optional[str] = None) -> List[dict]:
        ...
        filtered = frame[mask]
        return self._frame_to_records("AxiomWorkqueue", filtered)
