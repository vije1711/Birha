...
class GrammarApp:
    ...
    def show_continue_incomplete(self):
        ...
        try:
            ...
            pass
        self._wm_enable_safe_maximize(win, 0)
    ...
    def _wm_enable_safe_maximize(self, win, bottom_margin_px: int = 0) -> WindowManager | None:
        """Attach WindowManager and enable safe maximize after the window realizes."""
        mgr = self._wm_for(win)
        if not mgr:
            return None
        ...
        def _apply():
            try:
                mgr.enable_safe_maximize(bottom_margin_px)
            except Exception:
                pass
