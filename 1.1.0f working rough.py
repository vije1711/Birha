...
class GrammarApp:
    ...
    def split_meanings_for_display(self, meanings): # Helper function to split meanings into two columns
        left = meanings[:len(meanings)//2]
        right = meanings[len(meanings)//2:]
    ...
    def show_matches(self, matches, pankti, meanings, max_display=30):
        ...
        for i, column in enumerate(split_meanings.values()):
            ...
            for meaning in column:
                ...
                tk.Checkbutton(column_frame, text=f"- {meaning}", variable=var,
                            bg='light gray', font=('Arial', 12), wraplength=325,
                            anchor='w', justify=tk.LEFT, selectcolor='light blue').pack(anchor='w', padx=15, pady=5)
    ...
    def perform_search_and_finish(self):
        ...
        # Check if meanings are already accumulated
        if len(self.accumulated_meanings) > self.current_word_index:
            meanings = self.accumulated_meanings[self.current_word_index]
    ...
    def load_existing_assessment_data(self, file_path):
        expected_columns = [
            ...
            "Word", "Vowel Ending", "Number / ਵਚਨ", "Grammar / ਵਯਾਕਰਣ", "Gender / ਲਿੰਗ", "Word Root", "Word Type", "Word Index",
    ...
    def save_assessment_data(self, new_entry):
        ...
        - If any grammar field differs from the new entry, increment the revision number once for the group
            and update all matching rows with the new grammar details.
        - If no grammar differences are detected, no update is performed.
        - If no matching entry exists for the word in that verse, append the new entry with a revision of 1.
        """
        ...
        # Filter for rows with the same word and same verse.
        ...
        if not matching_rows.empty:
            # For repeated occurrences, pick the highest revision as a representative.
            latest_idx = matching_rows["Translation Revision"].idxmax()
            ...
            if differences:
                # Compute new revision number once for the group.
                new_revision = matching_rows["Translation Revision"].max() + 1
                new_entry["Translation Revision"] = new_revision
                
                # Update all matching rows with the new grammar values and new revision.
                for idx in matching_rows.index:
                    for key in grammar_keys:
                        df_existing.at[idx, key] = new_entry.get(key)
                    # Update the revision for the row.
                    df_existing.at[idx, "Translation Revision"] = new_revision
                    # Also update any additional fields from new_entry if needed.
            ...
            # No existing entry for this word in the verse; add it with revision 1.
    ...
    def compose_clipboard_text_for_chatgpt(self):
        ...
        for i, word in enumerate(current_verse_words):
            ...
            meanings_list = self.accumulated_meanings[actual_index] if actual_index < len(self.accumulated_meanings) else []
    ...
    def prompt_save_results(self, new_entries, skip_copy=False):
        ...
        for verse in self.selected_verses:
            ...
            # Create/update new entries for the current verse.
            # For each new entry, add the current verse.
            for entry in new_entries:
                # Only add the verse if the word appears in the current verse.
                if entry["Word"] in selected_words:
                    entry["Verse"] = self.accumulated_pankti

            # Filter new_entries to only those whose "Word" is present in the current verse.
            filtered_new_entries = [entry for entry in new_entries if entry["Word"] in selected_words]
    ...
    def accumulate_meanings_data(self, meanings):
        """
        Accumulate the meanings of the current word at its index in the pankti.

        Args:
        meanings (list): The list of meanings to be accumulated.
        """
        # Ensure the list is long enough to accommodate the current word index
        while len(self.accumulated_meanings) <= self.current_word_index:
            self.accumulated_meanings.append([])

        # Overwrite the meanings at the index of the current word
        self.accumulated_meanings[self.current_word_index] = meanings
    ...
    def start_progress(self):
        ...
        ttk.Label(self.progress_window, text="Processing...").pack(pady=10)
        ...
        self.root.update_idletasks()  # Force update so the progress window appears
